...
export class ReportStore {
  ...
  async save(report: DailyReport): Promise<void> {
    ...
    try {
      await this.storage.set(`${REDIS_KEYS.report}:${report.id}`, json, REPORT_TTL_SECONDS);
      // Also keyed by trading date, so a session can be looked up directly.
      await this.storage.set(`${REDIS_KEYS.reportDate}:${report.date}`, json, REPORT_TTL_SECONDS);
      await this.storage.set(REDIS_KEYS.reportLatest, json, REPORT_LATEST_TTL_SECONDS);
      await this.storage.listPush(REDIS_KEYS.reportHistory, JSON.stringify(meta));
      await this.storage.listTrim(REDIS_KEYS.reportHistory, REPORT_HISTORY_MAX);
  ...
  async getLatest(): Promise<DailyReport | null> {
    return this.read(REDIS_KEYS.reportLatest);
  }

  async getById(id: string): Promise<DailyReport | null> {
    return this.read(`${REDIS_KEYS.report}:${id}`);
  }

  async getByDate(date: string): Promise<DailyReport | null> {
    return this.read(`${REDIS_KEYS.reportDate}:${date}`);
  }

  async getHistory(limit = 30): Promise<ReportMeta[]> {
    const raw = await this.storage.listRange(REDIS_KEYS.reportHistory, limit);
