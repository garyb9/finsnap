...
import { CATEGORY_ORDER, DAILY_WINDOWS } from '../constants';
...
export interface CategoryLeaderboardCell {
  category: AssetCategory;
  /** How many assets of this category contributed a result */
  assetsCovered: number;
  winRatePct: number;
  avgExcessCagrPct: number;
}
...
export interface StrategyLeaderboardRow {
  ...
  /** Same pooling, sliced by asset category instead of by window — some categories (crypto,
   * bonds) are much thinner than others today, so one with only an asset or two still appears,
   * honestly labelled with its own `assetsCovered` rather than hidden. */
...
interface Accumulator {
  ...
  byCategory: Map<
    AssetCategory,
    { assets: Set<string>; beats: number; count: number; excessCagrSum: number }
  >;
}
...
function accumulate(report: DailyReport): Map<string, Accumulator> {
  ...
  for (const asset of report.assets) {
    for (const strategy of asset.daily) {
      ...
      let cAcc = acc.byCategory.get(asset.category);
      if (!cAcc) {
        cAcc = { assets: new Set(), beats: 0, count: 0, excessCagrSum: 0 };
        acc.byCategory.set(asset.category, cAcc);
      }
...
function toRow(acc: Accumulator): StrategyLeaderboardRow {
  return {
    ...
    byCategory: CATEGORY_ORDER.map((category): CategoryLeaderboardCell | null => {
      const c = acc.byCategory.get(category);
    ...
    }).filter((cell): cell is CategoryLeaderboardCell => cell !== null),
  };
}
