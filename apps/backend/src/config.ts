...
const configSchema = z.object({
  ...
  /**
   * Broad US and international indices, the eleven sector SPDRs, a few
   * industry-level slices (semiconductors, pharmaceuticals, and a broader
   * REIT fund alongside the real-estate sector SPDR), commodities,
   * currencies, credit, long bonds and a volatility proxy. Sector coverage is
   * deliberately complete: a partial set would make the breadth reading a
   * statement about which sectors were picked.
   */
  equitySymbols: z
    .string()
    .default(
      'SPY,QQQ,DIA,IWM,EEM,EFA,' +
        'XLK,XLF,XLE,XLV,XLI,XLY,XLP,XLU,XLB,XLRE,VNQ,XLC,' +
        'SMH,XPH,' +
        'GLD,SLV,USO,UNG,UUP,FXE,FXY,LQD,HYG,TLT,VXX,IBIT'
    ),

  // Options chains are only pulled for this subset (Yahoo chains are slow to
  // page). This is effectively the whole equity universe — every index,
  // sector, industry slice, commodity, bond and volatility proxy carries
  // real open interest. The one deliberate exclusion is the currency and
  // natural-gas funds (UNG, UUP, FXE, FXY): they list options, but thinly
  // enough that most expiries come back reading "thin" anyway.
  optionsSymbols: z
    .string()
    .default(
      'SPY,QQQ,DIA,IWM,EEM,EFA,' +
        'XLK,XLF,XLE,XLV,XLI,XLY,XLP,XLU,XLB,XLRE,VNQ,XLC,' +
        'SMH,XPH,' +
        'GLD,SLV,USO,TLT,LQD,HYG,VXX,IBIT'
    ),
